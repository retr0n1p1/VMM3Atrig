...
while (1)
{
	if((get_trigger_count != 1)&&(dataTakingState == 0)) clear_counter();
//	HAL_IWDG_Refresh(&hiwdg);
    ...
    if(is_trigger_detected()){
        testH.event = get_trigger_count();
        testH.time = 123;
        testH.bcid = get_last_trigger_time();
        __disable_irq();
        Spill.hits[Spill.hitCount] = testH;
        Spill.hitCount++; 
        for(size_t i = 0; i<Spill.hitCount; i++) Spill.hits[i].event = get_trigger_count();
        __enable_irq();
        DAQ_transmit(&Spill);
        clear_trigger_flag();
    }  
